...
一个基于 FastAPI 的前后端隔离的智能视频总结系统，支持视频上传、语音转写和AI文本总结功能。配置密钥教程：https://www.bilibili.com/video/BV1oApiziEn7/ 这是1.0版本的需要配置到环境变量，现在可以按下面的格式创建.env文件，将密钥粘贴进去即可。演示视频：https://www.bilibili.com/video/BV1ceCFBGECf/
...
### 用户管理
- 用户资料管理（头像、性别、生日等）
- 会员类型管理（普通用户/VIP用户）尚未实现，接入实际支付有点困难
...
| `id`          | 主键，自增用户ID        |
| `username`    | 用户名              |
| `email`       | 邮箱地址（用于登录或验证码发送） |
| `password`    | 用户密码（通常加密存储）     |
| `created_at`  | 创建时间             |
| `updated_at`  | 更新时间             |
| `member_type` | 会员类型（如普通用户、VIP等） |

---

### **2. user_profiles（用户资料表）**

**用途**：扩展用户的个性化资料，与 `users` 表通过 `user_id` 关联。
**字段说明**：

| 字段名          | 含义               |
| ------------ | ---------------- |
| `id`         | 主键               |
| `user_id`    | 外键，对应 `users.id` |
| `avatar_url` | 头像 URL           |
| `gender`     | 性别（可能是枚举或字符串）    |
| `birth_date` | 出生日期             |
...
### **3. verification_codes（邮箱验证码表）**

**用途**：保存邮箱验证码信息，用于注册、登录或找回密码等。
**字段说明**：
...
**用途**：存储用户的充值或付费订单信息，目前尚未实现沙盒支付，大家可以在这基础上增加支付功能。
**字段说明**：
...
| `order_no`       | 订单号（唯一）                     |
| `amount`         | 金额                          |
| `description`    | 订单描述（例如“购买会员月卡”）            |
| `status`         | 订单状态（如 pending、paid、failed） |
| `transaction_id` | 第三方支付平台的交易号                 |
| `created_at`     | 创建时间                        |
| `paid_at`        | 支付完成时间                      |
| `updated_at`     | 更新时间                        |
